document.addEventListener("DOMContentLoaded", () => {
    ...
    if (saveSettingsBtn) {
        saveSettingsBtn.addEventListener("click", () => {
            ...
            if (ws && ws.readyState === WebSocket.OPEN) {
                ...
                ws.send(payload);
                addLogEntry(`Applying system settings: Model -> ${newModel}, Persona -> ${newPersona}`, "system");
                settingsModal.classList.remove("active");
            }
        });
    }
    ...
    function renderGoalTree(goalsDict) {
        ...
        for (const goalId in goalsDict) {
            ...
            goalDiv.innerHTML = `<strong>Goal:</strong> ${goal.description} [${goal.status.toUpperCase()}]`;
    ...
    function renderLTM(memoryList) {
        ...
        memoryList.forEach(mem => {
            ...
            sourceSpan.textContent = `[${mem.source || 'unknown'}] `;

            const textSpan = document.createElement("span");
            textSpan.textContent = mem.text || "Empty memory node";

            memDiv.appendChild(scoreSpan);
            memDiv.appendChild(sourceSpan);
            memDiv.appendChild(textSpan);
